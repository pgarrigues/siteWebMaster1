...
<head>
	<meta charset="utf-8">
	<title>Établissements hospitaliers P2</title>
...
<body>
		...
		<div class="container">
			<div class="row">
				<div id="formulaire" class="col-md-4">
					<p> Trouvez un type spécifique d'établissement hospitalier dans votre département :</p>
					<?php
					$selectionType = "SELECT DISTINCT TYPE_ETABLISSEMENT FROM etab_hospitaliers ORDER BY TYPE_ETABLISSEMENT ASC";
					$resultatSelectionType = mysqli_query($connexion, $selectionType);
					$selectionDepartement = "SELECT DISTINCT DEPT FROM etab_hospitaliers ORDER BY DEPT ASC";
					$resultatSelectionDepartement = mysqli_query($connexion, $selectionDepartement);
					?>
					<form action="#" method="post">
						<select name="typeChoixHosp">
							<?php while($row1 = mysqli_fetch_array($resultatSelectionType)):; ?>
								<option value="<?php echo $row1[0]?>"> <?php echo $row1[0]; ?> </option>
							<?php endwhile; ?>
						</select>
						<select name="departementChoixHosp">
					...
					if(isset($_POST['submit'])){
						$selected_type = $_POST['typeChoixHosp'];
						$selected_departement = $_POST['departementChoixHosp'];
					...
					if(isset($selected_type)){
						if(isset($selected_departement)){
							$selectionHosp = "SELECT etab_hospitaliers.RAISON_SOCIALE AS Nom, etab_hospitaliers.ADRESSE_COMPLETE AS Adresse FROM etab_hospitaliers WHERE etab_hospitaliers.TYPE_ETABLISSEMENT = '$selected_type' AND etab_hospitaliers.DEPT = '$selected_departement' ORDER BY RAISON_SOCIALE";
							$resultatSelectionHosp = mysqli_query($connexion, $selectionHosp);
							$compteur = 1;
							?>
							<table>
								<?php
								echo "<br>";
								if ($resultatSelectionHosp == true){
						// Récupération de la première ligne --> entetes:
									$entete = mysqli_fetch_assoc($resultatSelectionHosp);
									echo "<tr>";
									...
									echo "</tr>";
									while ($ligne = mysqli_fetch_assoc($resultatSelectionHosp)){
										echo "<tr>";
	...
	<script type="text/javascript">
		...
		var listeEtabHosp = <?php echo json_encode($listeHospBis); ?>;
